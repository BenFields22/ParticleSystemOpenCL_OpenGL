...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Benjamin Fields" };
...
int stopFramesWatcher = 500;
const char *CL_FILE_NAME   = { "BenFields.cl" };
...
const char *CL_BINARY_NAME = { "BenFields.nv" };
...
double	ElapsedTime = 1000000000000000.0;//used to prevent the initialized value from dividing by zero
int		ShowPerformance;
float averagePerfromance = 0.0f;
...
Display( )
{
	...
	if( ShowPerformance )
	{
		averagePerfromance += (float)NUM_PARTICLES / (float)ElapsedTime / 1000000000.;
		char str[128];
		sprintf(str, "Frame %8d -- %6.1f GigaParticles/Sec", Frame, averagePerfromance / (float)Frame );
...
Quit( )
{
	...
	outFile<< NUM_PARTICLES <<","<< averagePerfromance / (float)Frame << "\n";
	outFile.close();
